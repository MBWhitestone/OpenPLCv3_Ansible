...
ADD_REQUIRED_PROPERTIES = ['slave_polling_period',
                           'slave_timeout', 'auto_run_text']
ID_COLUMN = 1
SELECTED_ITEM = ''
MISSING_ENTRIES = []
#####################################
...
class ActionModule(ActionBase):
    ...
    def _add(self, item, page=ADD):
        ...
        page. The _modify and _remove are not used in this Class.
        """
        assert PROPERTIES in self.args, 'Missing properties'
        details = self._details(None)
        checked = [[name for name in details.keys()
                    if re.findall('id=".*?"', r)[0][4:-1] in name][0]
                   for r in re.findall('<input .*?>',
                                       self._get(self.url + page).text, re.DOTALL)
                   if 'checked' in r]
        print('Checked', checked)
        current_details = {k: details[k]
                           for k in ADD_REQUIRED_PROPERTIES + checked}

        # Actual modification of settings.
        changed = False
        if PROPERTIES in self.args:
            for p, value in self.args[PROPERTIES].items():
                if p not in details:
                ...
                if value in ['false', 'none']:
                    if p in current_details:
                        print('removing', p)
                        del current_details[p]
                        changed = True
                elif current != value or p not in current_details:
                    if current != value:
                        print('modifing', p, current, 'to', value)
                    else:
                        print('enabling', p, 'with value', value)
                    current_details[p] = value
                    changed = True
        if changed:
            self._post(self.url + page, data=current_details)
        return changed
